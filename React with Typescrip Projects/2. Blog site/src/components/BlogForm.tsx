...
const BlogForm: React.FC<BlogFormProps> = ({ existingBlog, onClose }) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>): void => {
    ...
    if (file) {
      ...
      reader.onload = () => {
        console.log("File data:", reader.result); // This will be the image data URL
        setImage(reader.result as string); // Assuming `setImage` stores the uploaded image
      };
  ...
  return (
    <div className="bg-white p-6 rounded-lg w-[30rem] mx-auto">
      ...
      <section className="flex justify-end mt-6 space-x-4">
        ...
        <button
          onClick={onClose}
          className="bg-gray-500 text-white px-6 py-2 rounded-lg font-semibold shadow-sm hover:bg-gray-600 focus:outline-none focus:ring-gray-500"
        >
          Cancel
        </button>
      </section>
